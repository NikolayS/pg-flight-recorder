...
CREATE OR REPLACE FUNCTION pgfr._compare(
...
CREATE OR REPLACE FUNCTION pgfr._recent_waits_current()
RETURNS TABLE (
...
CREATE OR REPLACE FUNCTION pgfr._recent_activity_current()
RETURNS TABLE (
    captured_at TIMESTAMPTZ,
    pid INTEGER,
    usename TEXT,
...
CREATE OR REPLACE FUNCTION pgfr._recent_locks_current()
RETURNS TABLE (
    captured_at TIMESTAMPTZ,
    blocked_pid INTEGER,
    blocked_user TEXT,
...
CREATE OR REPLACE FUNCTION pgfr._wait_summary(
    p_start_time TIMESTAMPTZ,
    p_end_time TIMESTAMPTZ
)
RETURNS TABLE(
...
CREATE OR REPLACE FUNCTION pgfr._statement_compare(
    p_start_time TIMESTAMPTZ,
    p_end_time TIMESTAMPTZ,
...
CREATE OR REPLACE FUNCTION pgfr._activity_at(p_timestamp TIMESTAMPTZ)
RETURNS TABLE(
...
CREATE OR REPLACE FUNCTION pgfr._detect_query_storms(
...
END;
$$;
COMMENT ON FUNCTION pgfr._detect_query_storms(INTERVAL, NUMERIC) IS 'Detect query storms by comparing recent execution counts to baseline. Classifies as RETRY_STORM, CACHE_MISS, SPIKE, or NORMAL with severity levels (LOW, MEDIUM, HIGH, CRITICAL).';

-- Diagnose probable causes for a query's performance regression
-- Analyzes pg_stat_statements and snapshots for indicators
CREATE OR REPLACE FUNCTION pgfr._diagnose_regression_causes(
    p_queryid BIGINT
)
RETURNS TEXT[]
LANGUAGE plpgsql STABLE AS $$
...
END;
$$;
COMMENT ON FUNCTION pgfr._diagnose_regression_causes(BIGINT) IS 'Internal: Analyze a query to suggest probable causes for performance regression.';
...
CREATE OR REPLACE FUNCTION pgfr._detect_regressions(
    p_lookback INTERVAL DEFAULT NULL,
...
BEGIN
    ...
    SELECT
        ...
        pgfr._diagnose_regression_causes(reg.queryid) AS probable_causes
...
END;
$$;
COMMENT ON FUNCTION pgfr._detect_regressions(INTERVAL, NUMERIC) IS 'Detect performance regressions using buffer metrics (default) or timing. Classifies severity based on percentage change (LOW <200%, MEDIUM <500%, HIGH <1000%, CRITICAL >1000%). Configure via regression_detection_metric.';
