...
SELECT has_function('pgfr', 'disable', 'Function pgfr.disable should exist');

-- Test enable() function exists
SELECT has_function('pgfr', 'enable', 'Function pgfr.enable should exist');
...
SELECT ok(
    EXISTS (SELECT 1 FROM pgfr.config WHERE key = 'circuit_breaker_enabled'),
    'P0 Safety: Circuit breaker config should exist'
);

SELECT ok(
    EXISTS (SELECT 1 FROM pgfr.config WHERE key = 'circuit_breaker_threshold_ms'),
...
SELECT ok(
    EXISTS (SELECT 1 FROM pgfr.collection_stats WHERE collection_type = 'sample'),
    'P0 Safety: Collection stats should be recorded for sample()'
);

SELECT ok(
    (SELECT success FROM pgfr.collection_stats WHERE collection_type = 'sample' ORDER BY started_at DESC LIMIT 1) = true,
...
SELECT ok(
    EXISTS (SELECT 1 FROM pgfr.collection_stats WHERE collection_type = 'snapshot'),
    'P0 Safety: Collection stats should be recorded for snapshot()'
);

SELECT ok(
    (SELECT success FROM pgfr.collection_stats WHERE collection_type = 'snapshot' ORDER BY started_at DESC LIMIT 1) = true,
...
DELETE FROM pgfr.collection_stats WHERE collection_type = 'sample';

-- Insert a fake long-running collection (most recent)
INSERT INTO pgfr.collection_stats (collection_type, started_at, completed_at, duration_ms, success)
...
-- Reset threshold
UPDATE pgfr.config SET value = '5000' WHERE key = 'circuit_breaker_threshold_ms';
...
SELECT lives_ok(
    $$SELECT * FROM pgfr.cleanup('1 day')$$,
...
SELECT ok(
    EXISTS (SELECT 1 FROM pgfr.config WHERE key = 'retention_samples_days'),
    'P2: Samples retention config should exist'
);

SELECT ok(
    EXISTS (SELECT 1 FROM pgfr.config WHERE key = 'retention_snapshots_days'),
    'P2: Snapshots retention config should exist'
);

SELECT ok(
    EXISTS (SELECT 1 FROM pgfr.config WHERE key = 'retention_statements_days'),
    'P2: Statements retention config should exist'
);

-- Test P2: cleanup() now returns 3 columns (removed vacuumed_tables)
SELECT lives_ok(
    $$SELECT deleted_snapshots, deleted_samples, deleted_statements FROM pgfr.cleanup()$$,
...
SELECT has_function(
    'pgfr', 'health_check',
    'P3: Function pgfr.health_check should exist'
);
...
SELECT ok(
    EXISTS (SELECT 1 FROM pgfr.config WHERE key = 'alert_enabled'),
    'P4: Alert enabled config should exist'
);

SELECT ok(
    EXISTS (SELECT 1 FROM pgfr.config WHERE key = 'alert_circuit_breaker_count'),
    'P4: Alert circuit breaker count config should exist'
);

SELECT ok(
    EXISTS (SELECT 1 FROM pgfr.config WHERE key = 'alert_schema_size_mb'),
...
SELECT ok(
    (SELECT value FROM pgfr.config WHERE key = 'alert_enabled') = 'false',
...
SELECT ok(
    (SELECT pgfr_analyze.report(now() - interval '1 hour', now()) LIKE
            '%**Version:** ' || (SELECT value FROM pgfr.config WHERE key = 'schema_version') || '%'),
...
SELECT has_function(
    'pgfr', 'config_recommendations',
    'P4: Function pgfr.config_recommendations should exist'
);

-- Test P4: Config recommendations returns results
SELECT ok(
    (SELECT count(*) FROM pgfr.config_recommendations()) >= 1,
...
SELECT has_function(
    'pgfr', 'get_current_profile',
    'Profiles: Function get_current_profile should exist'
);

-- Test list_profiles returns expected profiles
SELECT ok(
    (SELECT count(*) FROM pgfr.list_profiles()) = 5,
    'Profiles: list_profiles should return 5 profiles'
);

SELECT ok(
    (SELECT count(*) FROM pgfr.list_profiles() WHERE profile_name = 'default') = 1,
    'Profiles: default profile should exist'
);

SELECT ok(
    (SELECT count(*) FROM pgfr.list_profiles() WHERE profile_name = 'production_safe') = 1,
    'Profiles: production_safe profile should exist'
);

SELECT ok(
    (SELECT count(*) FROM pgfr.list_profiles() WHERE profile_name = 'development') = 1,
    'Profiles: development profile should exist'
);

SELECT ok(
    (SELECT count(*) FROM pgfr.list_profiles() WHERE profile_name = 'troubleshooting') = 1,
    'Profiles: troubleshooting profile should exist'
);

SELECT ok(
    (SELECT count(*) FROM pgfr.list_profiles() WHERE profile_name = 'minimal_overhead') = 1,
    'Profiles: minimal_overhead profile should exist'
);

-- Test explain_profile works for each profile
SELECT lives_ok(
    $$SELECT * FROM pgfr.explain_profile('default')$$,
...
SELECT ok(
    (SELECT count(*) FROM pgfr.explain_profile('default')) > 5,
...
SELECT ok(
    (SELECT value FROM pgfr.config WHERE key = 'sample_interval_seconds') = '300',
    'Profiles: production_safe should set sample_interval_seconds to 300'
);

SELECT ok(
    (SELECT value FROM pgfr.config WHERE key = 'enable_locks') = 'false',
...
SELECT throws_ok(
    $$SELECT * FROM pgfr.explain_profile('invalid_profile')$$,
    'Unknown profile: invalid_profile. Run pgfr.list_profiles() to see available profiles.',
    'Profiles: explain_profile should reject invalid profile name'
);

-- Test apply_profile rejects invalid profile
SELECT throws_ok(
    $$SELECT * FROM pgfr.apply_profile('invalid_profile')$$,
    'Unknown profile: invalid_profile. Run pgfr.list_profiles() to see available profiles.',
