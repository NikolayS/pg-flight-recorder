...
SELECT lives_ok(
    $$SELECT * FROM pgfr.compare('2025-12-31', '2024-01-01')$$,
    'Boundary: compare() with backwards range should not crash'
);

-- Test compare() with future dates
SELECT lives_ok(
    $$SELECT * FROM pgfr.compare(now() + interval '1 year', now() + interval '2 years')$$,
    'Boundary: compare() with future dates should not crash'
);

-- Test compare() with very old dates (epoch)
SELECT lives_ok(
    $$SELECT * FROM pgfr.compare('1970-01-01'::timestamptz, '1970-01-02'::timestamptz)$$,
...
BEGIN
    ...
    IF v_start IS NOT NULL THEN
        PERFORM * FROM pgfr.wait_summary(v_start, v_end);
    END IF;
END $$;
...
SELECT ok(
    (SELECT count(*) FROM pgfr.recent_waits_current()) >= 0,
...
SELECT ok(
    EXISTS(
        SELECT 1 FROM pgfr.recent_waits_current()
        WHERE captured_at IS NOT NULL
        LIMIT 1
    ) OR NOT EXISTS(SELECT 1 FROM pgfr.recent_waits_current()),
...
SELECT ok(
    EXISTS(
        SELECT 1 FROM pgfr.recent_activity_current()
        WHERE captured_at IS NOT NULL
        LIMIT 1
    ) OR NOT EXISTS(SELECT 1 FROM pgfr.recent_activity_current()),
    'Real-Time: recent_activity_current() should have captured_at column'
);

-- Test recent_locks_current() with current data
SELECT ok(
    (SELECT count(*) FROM pgfr.recent_locks_current()) >= 0,
...
SELECT ok(
    EXISTS(
        SELECT 1 FROM pgfr.recent_locks_current()
        WHERE captured_at IS NOT NULL
        LIMIT 1
    ) OR NOT EXISTS(SELECT 1 FROM pgfr.recent_locks_current()),
...
SELECT lives_ok(
    $$SELECT * FROM pgfr.recent_waits_current()$$,
...
SELECT lives_ok(
    $$SELECT
        (SELECT count(*) FROM pgfr.recent_waits_current()) +
        (SELECT count(*) FROM pgfr.recent_activity_current()) +
        (SELECT count(*) FROM pgfr.recent_locks_current())
    $$,
