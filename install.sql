...
CREATE OR REPLACE FUNCTION pgfr.compare(
...
CREATE OR REPLACE FUNCTION pgfr.recent_waits_current()
RETURNS TABLE (
...
CREATE OR REPLACE FUNCTION pgfr.recent_activity_current()
RETURNS TABLE (
    captured_at TIMESTAMPTZ,
    pid INTEGER,
    usename TEXT,
...
CREATE OR REPLACE FUNCTION pgfr.recent_locks_current()
RETURNS TABLE (
    captured_at TIMESTAMPTZ,
    blocked_pid INTEGER,
    blocked_user TEXT,
...
CREATE OR REPLACE FUNCTION pgfr.wait_summary(
    p_start_time TIMESTAMPTZ,
    p_end_time TIMESTAMPTZ
)
RETURNS TABLE(
...
CREATE OR REPLACE FUNCTION pgfr.statement_compare(
    p_start_time TIMESTAMPTZ,
    p_end_time TIMESTAMPTZ,
...
CREATE OR REPLACE FUNCTION pgfr.activity_at(p_timestamp TIMESTAMPTZ)
RETURNS TABLE(
...
BEGIN
    ...
    RAISE NOTICE '     SELECT * FROM pgfr.compare(''2024-12-16 14:00'', ''2024-12-16 15:00'');';
    RAISE NOTICE '     SELECT * FROM pgfr.wait_summary(''2024-12-16 14:00'', ''2024-12-16 15:00'');';
    RAISE NOTICE '';
